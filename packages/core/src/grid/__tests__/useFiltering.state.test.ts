...
describe('useFiltering.state - converted legacy behavior', () => {
  describe('#changeColumnFilter', () => {
    it('should work', () => {
      ...
      expect((changeColumnFilter as any)(state, payload))
        .toEqual({ filters: [payload] });
    });
    ...
    it('should add new filter if keepOther filters', () => {
      ...
      const payload = { columnName: 'test2', config: { operation: 'contains', value: 'b' }, keepOther: true } as any;
      expect((changeColumnFilter as any)(state, payload))
        .toEqual({ filters: [
          { columnName: 'test', config: { operation: 'contains', value: 'a' } },
