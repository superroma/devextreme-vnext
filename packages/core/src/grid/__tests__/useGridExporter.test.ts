...
describe('useGridExporter - converted legacy behavior', () => {
  ...
  describe('#groupTree', () => {
    describe('without grouping', () => {
      it('flat rows', () => {
        ...
        expect(buildGroupTree?.(rows, {}, undefined, isGroupRow, undefined))
          .toEqual({ [ROOT_GROUP]: [0, 2] });
      });
    });
  });
  ...
  describe('#rowsToExport', () => {
    ...
    it('plain rows', () => {
      const rows = [{}, {}, {}];
      const getCollapsedRows = () => null; const getRowId = () => null;
      const { rowsToExport } = (useGridExporter as any) || {};
      expect(rowsToExport?.(rows, null, grouping, getCollapsedRows, getRowId, isGroupRow)).toEqual(rows);
    });
  });
  ...
  describe('#exportSummaryGetter', () => {
    ...
    const excelColumns = tableColumns.reduce((acc, { column: { name } }) => ({ ...acc, [name]: { letter: name.toUpperCase() } }), {} as any);
    ...
    let cells; beforeEach(() => { cells = { a: {}, b: {}, g: {} }; worksheet.lastRow.getCell.mockImplementation((n) => cells[n]); });
    it('export summary', () => {
      const { exportSummaryGetter } = (useGridExporter as any) || {};
      const exportSummary = exportSummaryGetter?.(worksheet, tableColumns, customizeSummaryCell, defaultMessages);
  ...
  describe('#closeGroupGetter', () => {
    ...
    const groupTree = { 1: ['1|1', '1|2'], '1|1': ['1|1|a', '1|1|b'], '1|1|a': [3, 5], '1|1|b': [7, 11], '1|2': ['1|2|c'], '1|2|c': [13, 15] };
    const outlineLevels = { a: 0, b: 1, c: 2 };
    const groupSummaryItems = [{ columnName: 'a', type: 'sum' }, { columnName: 'b', type: 'max' }];
    ...
    it('export group summaries', () => {
      ...
      const closeGroup = closeGroupGetter?.(worksheet, groupTree, outlineLevels, 2, groupSummaryItems, exportSummary)?.(10);
