  ...
  describe('#customGroupedRows', () => {
    it('should process hierarchical data by one column', () => {
      const hierarchicalSource = [
        { key: 1, items: [{ a: 1, b: 1 }, { a: 1, b: 2 }] },
        { key: 2, items: [{ a: 2, b: 1 }, { a: 2, b: 2 }] },
        { key: 3, items: [] },
      ];
      const getHierarchicalChildGroups = groups => groups
        .map(g => ({ key: String(g.key), value: g.key, childRows: g.items }));
      ...
      const { customGroupedRows } = { customGroupedRows: (rows, g, getter) => (useCustomGrouping as any)?.customGroupedRows?.(rows, g, getter) } as any;
      expect(customGroupedRows(hierarchicalSource, groupings, getChildGroups)).toEqual(groupedRows);
      expect(getChildGroups).toHaveBeenCalledWith(hierarchicalSource, groupings[0], hierarchicalSource);
    });

    it('should process hierarchical data by several columns', () => {
      const hierarchicalSource = [{
        key: 1,
        items: [{ key: 1, items: [{ a: 1, b: 1 }] }, { key: 2, items: [{ a: 1, b: 2 }] }],
      }, {
        key: 2,
        items: [{ key: 1, items: [{ a: 2, b: 1 }] }, { key: 2, items: [{ a: 2, b: 2 }] }],
      }];
      const getHierarchicalChildGroups = groups => groups
        .map(g => ({ key: String(g.key), value: g.key, childRows: g.items }));
      ...
      expect(customGroupedRows(hierarchicalSource, groupings, getChildGroups)).toEqual(groupedRows);
      expect(getChildGroups).toHaveBeenCalledWith(hierarchicalSource, groupings[0], hierarchicalSource);
      expect(getChildGroups).toHaveBeenCalledWith(hierarchicalSource[0].items, groupings[1], hierarchicalSource);
      expect(getChildGroups).toHaveBeenCalledWith(hierarchicalSource[1].items, groupings[1], hierarchicalSource);
    });

    it('should process hierarchical data with remote expanded groups', () => {
      const hierarchicalSource = [{ key: 1, items: null }, { key: 2, items: [] }];
      const getHierarchicalChildGroups = groups => groups
        .map(g => ({ key: String(g.key), value: g.key, childRows: g.items }));
      const groupings = [{ columnName: 'a' }, { columnName: 'b' }];
      const groupedRows = [
