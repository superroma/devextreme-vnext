...
export function useSorting<T extends Row>(data: T[], options: UseSortingOptions<T> = {}) {
  const {
  ...
  const getColumnCompare = useCallback(
    (name: string) => columnExtensions.find((c) => c.columnName === name)?.compare,
    [columnExtensions]
  )
  ...
  const sortedData = useMemo(() => {
    ...
    return sortedRows(data, effectiveSorting, getCellValue, getColumnCompare) as T[]
