...
describe('useSorting.state - converted legacy behavior', () => {
  describe('#changeColumnSorting', () => {
    it('can initiate sorting', () => {
      ...
      expect((changeColumnSorting as any)(state, payload))
        .toEqual({ sorting: [{ columnName: 'test', direction: 'asc' }] });
    });
    ...
    it('can initiate sorting with direction', () => {
      ...
      expect((changeColumnSorting as any)(state, payload))
        .toEqual({ sorting: [{ columnName: 'test', direction: 'desc' }] });
    });
    ...
    it('should reset sorting if no keepOther is specified', () => {
      ...
      expect((changeColumnSorting as any)(state, payload))
        .toEqual({ sorting: [{ columnName: 'test2', direction: 'asc' }] });
    });
    ...
    it('should work with immutable properties', () => {
      const state = { sorting: [{ columnName: 'test', direction: 'asc' }] } as any;
      expect(() => (changeColumnSorting as any)(state, { columnName: 'test2', keepOther: true })).not.toThrow();
      expect(() => (changeColumnSorting as any)(state, { columnName: 'test2', keepOther: ['test'] })).not.toThrow();
    });
