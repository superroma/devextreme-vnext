...
export function startEditRows(
  state: { editingRowIds: (string | number)[] },
...
export function changeAddedRow(
  state: { addedRows: any[] },
  payload: { rowId: number; change: any }
) {
...
export function cancelAddedRows(state: { addedRows: any[] }, payload: { rowIds: number[] }) {
  ...
  const result: any[] = []
  state.addedRows.forEach((row, idx) => {
    if (!set.has(idx)) result.push(row)
  })
  return { addedRows: result }
}

export function changeRow(
  state: { rowChanges: Record<string | number, any> },
  payload: { rowId: string | number; change: any }
) {
...
export function deleteRows(
  ...
  payload.rowIds.forEach((id) => {
    delete nextRowChanges[id]
  })
  return { deletedRowIds: nextDeleted, rowChanges: nextRowChanges, addedRows: [] }
}
export function cancelDeletedRows(
  state: { deletedRowIds: (string | number)[] },
  payload: { rowIds: (string | number)[] }
) {
  const set = new Set(payload.rowIds)
...
export function useEditingState(options: UseEditingStateOptions = {}) {
  const {
    editingRowIds: cEditingRowIds,
    onEditingRowIdsChange,
    defaultEditingRowIds = [],
    addedRows: cAddedRows,
    onAddedRowsChange,
    defaultAddedRows = [],
    rowChanges: cRowChanges,
    onRowChangesChange,
    defaultRowChanges = {},
    deletedRowIds: cDeletedRowIds,
  ...
  const setEditingRowIds = useCallback(
    (next: (string | number)[]) => {
  ...
  const setAddedRows = useCallback(
    (next: any[]) => {
