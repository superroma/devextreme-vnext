...
describe('useGridExporter.helpers - converted legacy behavior', () => {
  ...
  const { findRanges, exportHeader, exportRows, closeSheet, normalizeRanges, removeEmptyGroups } = helpers;

  describe('#exportHeader', () => {
    it('should work', () => {
      const worksheet = { addRow: vi.fn(), views: [], columns: null, lastRow: { number: 'last' } };
      const columns = [ { column: { name: 'a', title: 'Column A' } }, { column: { name: 'b', title: 'Column B' }, width: 200 } ];
      exportHeader?.(worksheet, columns);
      expect(worksheet.columns).toEqual([{ key: 'a', width: 18.75 }, { key: 'b', width: 25 }]);
  ...
  describe('#findRanges', () => {
    const groupTree = { [ROOT_GROUP]: ['group1', 'group2'], group1: ['group1|nested1', 'group1|nested2'], 'group1|nested1': [4, 10], 'group1|nested2': [12, 15], group2: ['group2|nested1', 'group2|nested2'], 'group2|nested1': [18, 23], 'group2|nested2': [25, 30] };
    it('range for flat rows', () => { expect(findRanges?.({ [ROOT_GROUP]: [5, 20] }, ROOT_GROUP, 0, 0)).toEqual([[5, 20]]); });
  });
  ...
  describe('#exportRows', () => {
    ...
    let worksheet;
    beforeEach(() => { getCell.mockReturnValue({}); worksheet = { addRow: vi.fn(), mergeCells: vi.fn(), lastRow: { number: 'last index', getCell, eachCell } }; });
    it('data rows', () => {
      const rows = [{ a:1,b:2,c:3 },{ a:4,b:5,c:6 }];
      exportRows?.(worksheet, rows, dataColumns, tableColumns, isGroupRow, {}, 0, getCellValue, getCloseGroup, customizeCell);
  ...
  describe('#closeSheet', () => {
    it('total summaries', () => {
      ...
      const totalSummaryItems = [{ columnName: 'a', type: 'sum' }, { columnName: 'b', type: 'count' }];
  ...
  describe('#normalizeRanges', () => {
    it('apply offset', () => { expect(normalizeRanges?.([[0,3],[10,15]], 5)).toEqual([[5,8],[15,20]]); });
  });
