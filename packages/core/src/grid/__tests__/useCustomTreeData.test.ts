...
describe('useCustomTreeData - converted legacy behavior', () => {
  describe('#customTreeRowsWithMeta', () => {
    it('should process hierarchical data', () => {
      ...
      ;[null, ...linearizedRows.rows].forEach((r) =>
        expect(getChildRows).toHaveBeenCalledWith(r, hierarchicalSource)
      )
    })
    ...
    it('should process plain data', () => {
      ...
      ;[null, ...linearizedRows.rows].forEach((r) =>
        expect(getChildRows).toHaveBeenCalledWith(r, plainSource)
      )
    })
  ...
  describe('#customTreeRowLevelKeyGetter', () => {
    ...
    it('should define row levels', () => {
      ...
      const getRowLevelKey =
        customTreeRowLevelKeyGetter?.(parentGetRowLevelKey, linearizedRows) || (() => undefined)
    ...
    it('should provide row levels for unknown rows', () => {
      ...
      const { customTreeRowLevelKeyGetter } = (useCustomTreeData as any) || {}
      const getRowLevelKey =
        customTreeRowLevelKeyGetter?.(parentGetRowLevelKey, linearizedRows) || (() => 0)
  ...
  describe('#expandedTreeRows', () => {
    ...
    it('should collapse rows', () => {
      ...
      expect(expandedTreeRows?.(linearizedRows, getRowId, expandedRowIds)).toEqual(
        expandedLinearizedRows
      )
    })
  })
  ...
  describe('#collapsedTreeRowsGetter', () => {
    ...
    it('should define collapsed rows', () => {
      ...
      const getCollapsedRows =
        collapsedTreeRowsGetter?.(undefined, linearizedRows) || (() => undefined)
    ...
    it('should provide row levels for unknown rows', () => {
      ...
      const getCollapsedRows =
        collapsedTreeRowsGetter?.(parentGetCollapsedRows, linearizedRows) || parentGetCollapsedRows
