...
// Phase: RED (Step 08) – Full 1:1 parity (no placeholder loosening)
import { describe, it, expect } from 'vitest'

// Real legacy semantics – import internal (to-be-implemented) helpers/constants
import {
  isNoDataColumn,
  isBandedTableRow,
  isBandedOrHeaderRow,
...
} from '../internal/table-band-header.constants'
import { TABLE_DATA_TYPE } from '../internal/table.constants'
import { TABLE_HEADING_TYPE } from '../internal/table-header-row.constants'
import { TABLE_STUB_TYPE } from '../internal/virtual-table.constants'
import { tableHeaderColumnChainsWithBands } from '../internal/table-band-header.computeds'
import { tableHeaderColumnChainsWithFixed } from '../internal/table-fixed-columns.computeds'
import { generateColumns } from '../internal/table-band-header.test-utils'

// NOTE: The internal modules above are expected to be stubbed/empty during RED; importing them is required for parity.
...
describe('useTableBandHeader.helpers - converted legacy behavior', () => {
  ...
  const computeColumnChains = (columns: any, rows: any, bands: any) => (
    tableHeaderColumnChainsWithBands(rows, columns, bands)
  )
  const computeColumnChainsWithFixed = (columns: any, rows: any, bands: any) => (
  ...
  describe('#isNoDataColumn', () => {
    it('should work with not data column', () => {
      expect(isNoDataColumn('editCommand')).toBeTruthy()
    })
    it('should work with data column', () => {
      expect(isNoDataColumn(TABLE_DATA_TYPE)).toBeFalsy()
    })
  })

  describe('#isBandedOrHeaderRow', () => {
    it('should work', () => {
      expect(isBandedOrHeaderRow({ type: TABLE_BAND_TYPE })).toBeTruthy()
      expect(isBandedOrHeaderRow({ type: TABLE_HEADING_TYPE })).toBeTruthy()
      expect(isBandedOrHeaderRow({ type: TABLE_DATA_TYPE })).toBeFalsy()
    })
  })

  describe('#isBandedTableRow', () => {
    it('should work', () => {
      expect(isBandedTableRow({ type: TABLE_BAND_TYPE })).toBeTruthy()
      expect(isBandedTableRow({})).toBeFalsy()
    })
  })

  describe('#getColumnMeta', () => {
    it('should return correct column meta for the first level children', () => {
      expect(getColumnMeta('d', columnBands as any, 1))
        .toEqual({ title: 'Band A', level: 1, key: '_Band A' })
    })

    it('should return correct column meta for the deeper children levels', () => {
      expect(getColumnMeta('a', columnBands as any, 2))
        .toEqual({ title: 'Band B', level: 2, key: '_Band A_Band B' })
    })

    it('should work with immutable properties', () => {
      // Immutable conversion not applied here (would require dependency); parity relies on non-throw behavior.
  ...
  describe('#getBandComponent', () => {
    ...
    describe('with virtual table', () => {
      ...
      describe('group cell colSpan', () => {
        ...
        const getParams = (tableColumn: any) => ({
          tableColumn,
          tableRow: { level: 0 },
        })
        const getBandComponentByViewport = (columnBoundaries: any, column: any) => getBandComponent(
          getParams(column), tableHeaderRows, testColumns, testBands, columnChains,
          columnBoundaries, levelsVisibility,
        )
        const groupCellWithColSpan = (colSpan: number) => ({
          type: BAND_GROUP_CELL,
          payload: {
            colSpan,
            value: 'Band0',
      ...
      describe('fill level cell', () => {
        ...
        const columnChains = computeColumnChains(testColumns, tableHeaderRows, testBands)
        const getParams = (tableColumn: any, level: any) => ({
          tableColumn,
          tableRow: { level },
        })
        const getBandComponentGetter = (columnBoundaries: any, testLevelsVisibility: any) => (
          (tableColumn: any, level: any) => (getBandComponent(
        ...
        it('should fill space vertically', () => {
          ...
          const assertEmptyCell = (level: number) => {
  ...
  describe('#calculateBand', () => {
    ...
    it('should return chain range if visibleBound is not defined', () => {
      expect(calculateBand(undefined as any, headerChain))
        .toEqual([1, 4])
    })
  })
})
